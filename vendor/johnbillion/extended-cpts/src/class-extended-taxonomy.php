	...
	public function register_taxonomy() {
		...
		if ( $query_var && count( $post_types ) ) {
			trigger_error( esc_html( sprintf(
				...
				__( 'Taxonomy query var "%s" clashes with a post type query var of the same name', 'rdc-business-hours' ),
				$query_var
			) ), E_USER_ERROR );
		} elseif ( in_array( $query_var, [ 'type', 'tab' ], true ) ) {
			trigger_error( esc_html( sprintf(
				/* translators: %s: Taxonomy query variable name */
				__( 'Taxonomy query var "%s" is not allowed', 'rdc-business-hours' ),
				$query_var
			) ), E_USER_ERROR );
		} else {
